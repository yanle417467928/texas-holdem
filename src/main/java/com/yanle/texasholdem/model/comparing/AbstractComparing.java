...
public abstract class AbstractComparing implements IComparing {
    ...
    protected int seqComparing(Player p1, Player p2) {
        List<Poker> p1Cards = p1.getPokers();
        List<Poker> p2Cards = p2.getPokers();

        int size = p1.getPokerSize();

        for (int i = 0; i < size; i++) {
            if (p1Cards.get(i).getNum().getNumber() < p2Cards.get(i).getNum().getNumber()) {
                return 1;
            }
            if (p1Cards.get(i).getNum().getNumber() > p2Cards.get(i).getNum().getNumber()) {
                return -1;
